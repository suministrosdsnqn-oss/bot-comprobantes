...
def es_duplicado(nombre_envia, monto, nro_comprobante):
    ...
    cur = conn.cursor()
    if nro_comprobante:
        cur.execute("SELECT fecha, hora FROM comprobantes WHERE nro_comprobante = %s AND ABS(COALESCE(monto_original, monto_egreso, 0) - %s) < 1",
            (str(nro_comprobante), monto))
    else:
        hoy = datetime.now().strftime("%Y-%m-%d")
        cur.execute("SELECT fecha, hora FROM comprobantes WHERE LOWER(envia) = LOWER(%s) AND ABS(COALESCE(monto_original, monto_egreso, 0) - %s) < 1 AND fecha = %s",
