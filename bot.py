...
def es_duplicado(nombre_envia, monto, nro_comprobante):
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT fecha, hora FROM comprobantes WHERE nro_comprobante = %s AND LOWER(envia) = LOWER(%s) AND ABS(COALESCE(monto_original, monto_egreso, 0) - %s) < 1",
...
async def procesar_y_guardar(update, datos_comp):
    ...
    if duplicado:
        saldo = cargar_saldo()
        lineas = ["*DUPLICADO - Transferencia ya registrada*", "Envia: " + nombre_envia, "Monto: " + formatear_pesos(monto_original), "Nro comprobante: " + str(nro_comprobante), "Registrada el: " + fecha_original, "*Saldo sin cambios: " + formatear_pesos(saldo) + "*"]
