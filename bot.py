...
def guardar_comprobante(tipo, envia, recibe, cuenta, monto_original, monto_neto, comision, monto_egreso, nro_comprobante):
    ...
    cur.execute("""
        INSERT INTO comprobantes (fecha, hora, tipo, envia, recibe, cuenta, monto_original, monto_neto, comision, monto_egreso, nro_comprobante)
...
def es_duplicado(nombre_envia, monto, nro_comprobante):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT fecha, hora FROM comprobantes
        WHERE nro_comprobante = %s AND LOWER(envia) = LOWER(%s)
        AND ABS(COALESCE(monto_original, monto_egreso, 0) - %s) < 1
    """, (str(nro_comprobante), nombre_envia, monto))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if row:
...
def get_comprobantes_hoy():
    ...
    cur.execute("SELECT * FROM comprobantes WHERE fecha = %s ORDER BY hora", (hoy,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return rows
...
def get_historial():
    ...
    cur.execute("""
        SELECT fecha,
            SUM(CASE WHEN tipo = 'ingreso' THEN monto_neto ELSE 0 END) as total_ingreso,
            SUM(CASE WHEN tipo = 'egreso' THEN monto_egreso ELSE 0 END) as total_egreso
        FROM comprobantes
...
def extraer_datos_imagen(image_data):
    ...
    msg = client.messages.create(
        ...
        messages=[{"role": "user", "content": [
            ...
            {"type": "text", "text": "Analiza este comprobante de transferencia bancaria. Extrae todos estos datos. Responde SOLO JSON sin texto extra: {\"nombre_envia\": \"...\", \"apellido_envia\": \"...\", \"nombre_recibe\": \"...\", \"apellido_recibe\": \"...\", \"monto\": 1234.0, \"nro_comprobante\": \"...\", \"cvu_destino\": \"...\"}. El nro_comprobante es el numero de operacion o identificador unico. El cvu_destino es el CVU o CBU del destinatario. Si no encuentras un dato usa null. El monto debe ser solo numeros sin puntos de miles ni comas."}
        ]}]
    )
...
async def procesar_y_guardar(update, datos_comp):
    ...
    if duplicado:
        saldo = cargar_saldo()
        lineas = [
            "*DUPLICADO - Transferencia ya registrada*",
            "Envia: " + nombre_envia,
    ...
    if recibe_titular and not envia_titular:
        ...
        lineas = [
            ...
            "De: " + nombre_envia,
            "Para: " + nombre_recibe,
            "Monto enviado: " + formatear_pesos(monto_original),
            "Nro comprobante: " + str(nro_comprobante or "no encontrado"),
            "*Saldo actual: " + formatear_pesos(saldo) + "*"
        ]
        await update.message.reply_text("\n".join(lineas), parse_mode="Markdown")
...
async def cmd_resumen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, c in enumerate(lista, 1):
        if c["tipo"] == "ingreso":
            ...
            lineas.append(str(i) + ". EGRESO a " + c["recibe"] + " - " + formatear_pesos(c["monto_egreso"]) + " (" + c["hora"] + ")")
    lineas.append("\n*Saldo actual: " + formatear_pesos(saldo) + "*")
    await update.message.reply_text("\n".join(lineas), parse_mode="Markdown")
...
async def cmd_historial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        lineas.append(r["fecha"] + ": +" + formatear_pesos(r["total_ingreso"]) + " / -" + formatear_pesos(r["total_egreso"]))
    lineas.append("\n*Saldo actual: " + formatear_pesos(saldo) + "*")
    await update.message.reply_text("\n".join(lineas), parse_mode="Markdown")
